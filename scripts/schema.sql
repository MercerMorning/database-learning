...
CREATE TABLE IF NOT EXISTS ozon.product (
                         id SERIAL PRIMARY KEY,
                         name VARCHAR(255) NOT NULL,
                         type_fk INTEGER REFERENCES type(id),
                         seller_fk INTEGER REFERENCES seller(id),
                         brand_fk INTEGER REFERENCES brand(id),
...
CREATE TABLE IF NOT EXISTS ozon."user" (
                        ...
                        basket_fk INTEGER REFERENCES basket(id)
...
CREATE TABLE IF NOT EXISTS ozon.attribute_type (
                                id SERIAL PRIMARY KEY,
                                type_fk INTEGER REFERENCES type(id),
                                attribute_fk INTEGER REFERENCES attribute(id)
);
...
CREATE TABLE IF NOT EXISTS ozon.order_product (
                               id SERIAL PRIMARY KEY,
                               order_fk INTEGER REFERENCES "order"(id),
                               product_fk INTEGER REFERENCES product(id),
                               price DECIMAL CHECK (price >= 0),
                               amount INTEGER CHECK (amount >= 0),
                               discount DECIMAL CHECK (discount >= 0),
...
CREATE TABLE IF NOT EXISTS ozon.delivery (
                          ...
                          address_delivery_fk INTEGER REFERENCES address_delivery(id),
                          grade INTEGER CHECK (grade >= 0 AND grade <= 5)
);
...
CREATE TABLE IF NOT EXISTS ozon.refund_product (
                                id SERIAL PRIMARY KEY,
                                refund_fk INTEGER REFERENCES refund(id),
                                product_fk INTEGER REFERENCES product(id)
);
...
CREATE TABLE IF NOT EXISTS ozon.tag_product (
                             id SERIAL PRIMARY KEY,
                             product_fk INTEGER REFERENCES product(id),
                             tag_fk INTEGER REFERENCES tag(id)
);
...
CREATE TABLE IF NOT EXISTS ozon.promotion_product (
                                   id SERIAL PRIMARY KEY,
                                   product_fk INTEGER REFERENCES product(id),
                                   promotion_fk INTEGER REFERENCES promotion(id)
);
...
CREATE TABLE IF NOT EXISTS ozon.gallery_reason (
                                id SERIAL PRIMARY KEY,
                                reason_fk INTEGER REFERENCES reason(id),
                                media_file_link TEXT CHECK (media_file_link ~* '^https?://.*')
    );
...
CREATE TABLE IF NOT EXISTS ozon.answer_review (
                               id SERIAL PRIMARY KEY,
                               review_fk INTEGER REFERENCES review(id),
                               answer_fk INTEGER REFERENCES answer(id)
);
...
CREATE TABLE IF NOT EXISTS ozon.question_product (
                                  id SERIAL PRIMARY KEY,
                                  product_fk INTEGER REFERENCES product(id),
                                  question_fk INTEGER REFERENCES question(id)
);
...
CREATE TABLE IF NOT EXISTS ozon.IF NOT EXISTS "user" (
                                      id SERIAL PRIMARY KEY,
...
CREATE INDEX idx_product_type ON product(type_fk);
CREATE INDEX idx_review_product ON review_product(product_fk);
CREATE INDEX idx_question_product ON question_product(product_fk);
